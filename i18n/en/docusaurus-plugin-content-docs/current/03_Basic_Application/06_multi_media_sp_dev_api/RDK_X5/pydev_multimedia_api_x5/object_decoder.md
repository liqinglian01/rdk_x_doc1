...
- RDK supports H.265/HEVC encoder, H.264/AVC encoder, H.265/HEVC decoder, H.264/AVC decoder, and JPEG and MJPEG encoder/decoder.
...
| list[1] | Number of frames in the input stream file, valid when decoding is successful |
...
When exiting the program, the `close` interface needs to be called to release resources.
...
This interface should be used after calling `Decoder.decode()` to create a decoding channel.
...
This interface should be used after calling `Decoder.decode()` to create a decoding channel, and the `file` parameter should be left empty when creating the decoding channel.
